...
plot_age_years_distribution <- function(df, by_group = NULL, min_age = NULL, max_age = NULL, file_path = NULL, wdth = NULL, hght = NULL, title_name = NULL) {
  ...
  if( !(c("age_years") %in% colnames(df))) {stop("There is no age_years column in your dataframe. Please check your inputs.")}
    ...
    if(is.null(by_group)) {
      g <- ggplot2::ggplot(data = df, ggplot2::aes(x = get("age_years") )) + ggplot2::geom_histogram(binwidth = 1) + ggplot2::scale_x_continuous(minor_breaks = seq(min_age,max_age,by=1), breaks = seq(min_age,max_age,by=1), limits = c(min_age,max_age))
    } else {
      g <- ggplot2::ggplot(data = df, ggplot2::aes(x = get("age_years"))) + ggplot2::geom_histogram(binwidth = 1) + ggplot2::scale_x_continuous(minor_breaks = seq(min_age,max_age,by=1), breaks = seq(min_age,max_age,by=1), limits = c(min_age,max_age)) + ggplot2::facet_wrap(~get(by_group), ncol = 1)
    }
