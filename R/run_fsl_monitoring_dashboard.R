...
#' Run FSL Monitoring Dashboard
#'
#' Runs a local shiny dashboard to explore data quality for FSL outcome indicators
#'
#' @param df A dataframe with FSL data that has been formatted by healthry::format_nut_health_indicators function.
...
#'
#' @examples
run_fsl_monitoring_dashboard <- function(df,
                                     grouping_var = NULL,
                                     filter_var1 = NULL,
                                     filter_var2 = NULL) {
  ...
  # before being used in the shinyApp function below.
   ...
   shiny::shinyApp(
    ...
    ui = shiny::fluidPage(theme = shinythemes::shinytheme("sandstone"),
      ...
      # Application title
      shiny::titlePanel("Monitoring Dashboard"),
    ...
    server = function(input, output, grp = grouping_var) {
      ...
      output$quality_table1 <- DT::renderDT({
          ...
          DT::datatable(healthyr::create_fsl_quality_report(df = reactive_db(), grouping = grouping_var, short_report = TRUE),
      ...
      output$quality_table2 <- DT::renderDT({
        ...
        DT::datatable(healthyr::create_fsl_quality_report(df = reactive_db(), grouping = grouping_var, short_report = FALSE),
                      filter = 'top',  extension = 'FixedColumns', options = list(scrollX = TRUE, fixedColumns = list(leftColumns = 2, rightColumns = 0)))
      ...
      output$test_plot5 <- renderPlot({
        ...
        cols <- intersect(c("fcs_score", "hhs_score", "rcsi_score", "hdds_score"), colnames(reactive_db()))
      ...
      output$test_plot6 <- renderPlot({
        ...
        cols <- intersect(c("fcs_cereal", "fcs_legumes", "fcs_dairy", "fcs_meat", "fcs_veg", "fcs_fruit", "fcs_oil", "fcs_sugar"), colnames(reactive_db()))

        healthyr::plot_ridge_distribution(df = reactive_db(), numeric_cols = cols)

  })

      output$test_plot7 <- renderPlot({

          cols <- intersect(c("rcsi_lesspreferred_1","rcsi_borrowfood_2","rcsi_limitportion_3","rcsi_restrict_4","rcsi_reducemeals5"), colnames(reactive_db()))
          healthyr::plot_ridge_distribution(df = reactive_db(), numeric_cols = cols)
      ...
      output$test_plot8 <- renderPlot({
        ...
        cols <- intersect(c("fcs_cereal", "fcs_legumes", "fcs_dairy", "fcs_meat", "fcs_veg", "fcs_fruit", "fcs_oil", "fcs_sugar"), colnames(reactive_db()))
        healthyr::plot_ridge_distribution(df = reactive_db(), numeric_cols = cols, grouping = grouping_var)
